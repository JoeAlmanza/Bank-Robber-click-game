...
let clickUpgrades = JSON.parse(localStorage.getItem("upgradeData")) || [
  { name: "Backpack", price: 40, quantity: 0, multiplier: 1 },
  { name: "Duffle", price: 200, quantity: 0, multiplier: 3 },
  { name: "Wheelbarrow", price: 500, quantity: 0, multiplier: 5 },
];
...
function buyClickUpgrade(ugName) {
  let clickUpgrade = clickUpgrades.find((u) => u.name == ugName);
...
function drawUpgrades() {
  ...
  clickUpgrades.forEach(
    (u) =>
      (template += `<button id="upgradeButton" onclick="buyClickUpgrade('${
        u.name
      }')"><b class="text-info">$${u.price} - ${u.name}</b> <br>x${
        u.quantity
      }  <br> <b class="text-success">Current Bonus:</b> $${
        u.quantity * u.multiplier
      }<br> every click</button>`)
  );

  automaticUpgrades.forEach(
    (u) =>
      (template += `<button id="upgradeButton" onclick="buyAutomaticUpgrade('${
        u.name
      }')"><b class="text-info">$${u.price} - ${u.name}</b> <br> x${
        u.quantity
      }  <br> <b class="text-success">Current Bonus:</b> $${
        u.quantity * u.multiplier
      }<br> every 3 seconds</button>`)
  );
